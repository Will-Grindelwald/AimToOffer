...
/**
 * 
 * 给定单向链表的头指针和一个结点指针，定义一个函数在 0(1) 时间删除该结点。
 * 
 */
// 别人的代码
// Java 写链表 链树 链图 的特点：因为可能要改第一个结点，而 Java 是传递引用，所以必须返回头指针（可能没改变也可能改变了）
public class T13DeleteNode {
	...
	/**
	 * 给定单向链表的头指针和一个结点指针，定义一个函数在 0(1) 时间删除该结点, 【注意 1：这个方法和文本上的不一样，书上的没有返回值，这个因为
	 * JAVA 引用传递的原因， 如果删除的结点是头结点，如果不采用返回值的方式，那么头结点永远删除不了】 【注意
	...
	public static ListNode deleteNode(ListNode head, ListNode toBeDeleted) {
		...
		// 下面的情况链表至少有两个结点
		if (toBeDeleted.next == null) { // 在多个节点的情况下，如果删除的是最后一个元素，只能顺序遍历一遍
			// 找待删除元素的前驱
			ListNode tmp = head;
